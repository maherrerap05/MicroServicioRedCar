...
namespace MicroServicio.RedCar.Business.Validators
{
    public static class VehiculoValidator
    {
        ...
        public static IReadOnlyCollection<string> ValidarActualizacion(ActualizarVehiculoRequest request)
        {
            ...
            // Strings: solo validan longitud si vienen con valor, pero si vienen vacíos también se rechazan
            if (request.codigo_interno_vehiculo != null)
            {
                if (string.IsNullOrWhiteSpace(request.codigo_interno_vehiculo))
                    errors.Add("El código interno del vehículo no puede ser vacío si se envía.");
                else if (request.codigo_interno_vehiculo.Trim().Length > 20)
                    errors.Add("El código interno del vehículo no puede exceder 20 caracteres.");
            }
            ...
            {
                if (string.IsNullOrWhiteSpace(request.placa_vehiculo))
                    errors.Add("La placa del vehículo no puede ser vacía si se envía.");
                else if (request.placa_vehiculo.Trim().Length > 15)
                    errors.Add("La placa del vehículo no puede exceder 15 caracteres.");
            }
            ...
            {
                if (string.IsNullOrWhiteSpace(request.modelo_vehiculo))
                    errors.Add("El modelo del vehículo no puede ser vacío si se envía.");
                else if (request.modelo_vehiculo.Trim().Length > 50)
                    errors.Add("El modelo del vehículo no puede exceder 50 caracteres.");
            }
            ...
            {
                if (string.IsNullOrWhiteSpace(request.color_vehiculo))
                    errors.Add("El color del vehículo no puede ser vacío si se envía.");
                else if (request.color_vehiculo.Trim().Length > 30)
                    errors.Add("El color del vehículo no puede exceder 30 caracteres.");
            }
            ...
            {
                if (string.IsNullOrWhiteSpace(request.tipo_combustible))
                    errors.Add("El tipo de combustible no puede ser vacío si se envía.");
                else if (request.tipo_combustible.Trim().Length > 20)
                    errors.Add("El tipo de combustible no puede exceder 20 caracteres.");
            }
            ...
            {
                if (string.IsNullOrWhiteSpace(request.tipo_transmision))
                    errors.Add("El tipo de transmisión no puede ser vacío si se envía.");
                else if (request.tipo_transmision.Trim().Length > 20)
                    errors.Add("El tipo de transmisión no puede exceder 20 caracteres.");
            }
            ...
            if (request.precio_base_dia.HasValue && request.precio_base_dia.Value <= 0)
                errors.Add("El precio base por día debe ser mayor que cero.");

            if (request.kilometraje_actual.HasValue && request.kilometraje_actual.Value < 0)
                errors.Add("El kilometraje actual no puede ser negativo.");
            ...
            {
                if (string.IsNullOrWhiteSpace(request.estado_vehiculo))
                    errors.Add("El estado del vehículo no puede ser vacío si se envía.");
                else if (request.estado_vehiculo != "ACT" && request.estado_vehiculo != "INA")
                    errors.Add("El estado del vehículo debe ser ACT o INA.");
            }
            ...
            // modificado_por_usuario y origen_registro los asigna el controller,
            // pero validamos por si acaso llegan con valor inválido
            ...
            // Reglas de inhabilitación — solo aplican si se está enviando el estado
            if (request.estado_vehiculo == "INA")
            {
                ...
                else if (request.motivo_inhabilitacion.Trim().Length > 200)
